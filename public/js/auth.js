...
function readStoredAuthUser() {
    ...
    if (rawLocal) {
        // Sincroniza para mantener compatibilidad con el flujo existente.
        sessionStorage.setItem('auth_user', rawLocal);
    }
...
loginForm.addEventListener('submit', async function (e) {
    ...
    try {
        const json = await postJSON(`${AUTH_API_BASE}/login`, { usuario, password });

        if (json.ok) {
            currentUser = json.user;
            const authData = JSON.stringify(json.user);
...
async function loadAuthData() {
    try {
        const json = await getJSON(`${AUTH_API_BASE}/data`);
        if (json.ok) {
...
window.deleteUsuarioBtn = async function (id) {
    ...
    try {
        const res = await fetch(`${AUTH_API_BASE}/usuarios/${id}`, {
            method: 'DELETE',
            credentials: 'same-origin',
            headers: { 'Accept': 'application/json' }
        });
...
document.getElementById('usuario-form').addEventListener('submit', async function (e) {
    ...
    try {
        const json = await postJSON(`${AUTH_API_BASE}/usuarios`, payload);
        if (json.ok) {
...
window.deletePerfilBtn = async function (id) {
    ...
    try {
        const res = await fetch(`${AUTH_API_BASE}/perfiles/${id}`, {
            method: 'DELETE',
            credentials: 'same-origin',
            headers: { 'Accept': 'application/json' }
        });
...
document.getElementById('perfil-form').addEventListener('submit', async function (e) {
    ...
    try {
        const json = await postJSON(`${AUTH_API_BASE}/perfiles`, payload);
        if (json.ok) {
