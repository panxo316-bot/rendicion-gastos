// URL_DEL_SCRIPT de Google Apps Script (Debe ser la de "Cualquier persona")
const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzCf2OAKCLmIiXOso42BamtZc4QAq8kkfGzikx9TUjmDCCl84a5W_iEyNfsOWOVVrucog/exec';
...
async function loadAllData() {
    if (GOOGLE_SCRIPT_URL.includes('pega_aqui') || GOOGLE_SCRIPT_URL === '') {
        console.warn("Falta URL. Usando modo offline / mock hasta que configuren.");
        familias = generarFamiliasVacias();
        loaderOverlay.style.display = 'none';
        renderConfig(); renderCuotas(); renderGastos();
        return;
    }
    ...
    try {
        ...
        if (json.ok) {
            ...
            configuracion = json.data.configuracion || configuracion;
...
pagoForm.addEventListener('submit', async (e) => {
    ...
    let payload = { action: 'add_pago', id_familia: fId, mes: mes, monto: monto };
    if (file) {
...
document.getElementById('btn-delete-pago').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch(GOOGLE_SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: 'delete_pago', id: currentPagoId }) });
        const json = await res.json();
        if (json.ok) {
            toast('Pago anulado');
...
gastoForm.addEventListener('submit', async (e) => {
    ...
    let payload = { action: 'add_gasto', descripcion, valor };
...
window.eliminarRegistro = async function (tipo, id) {
    ...
    const act = tipo === 'gasto' ? 'delete_gasto' : 'delete_pago';
    try {
        const res = await fetch(GOOGLE_SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: act, id: id }) });
