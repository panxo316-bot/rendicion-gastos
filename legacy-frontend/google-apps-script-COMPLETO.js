...
// ══════════════════════════════════════
// RESPALDO COMPLETO DESDE LARAVEL/SQLITE
// ══════════════════════════════════════
function handleFullBackup(data) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var timestamp = data.timestamp || Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "dd/MM/yyyy HH:mm:ss");

  function writeBackupSheet(name, headers, rows) {
    var sheetName = 'BK_' + name;
    ...
    sheet.appendRow(['# Respaldo: ' + timestamp]);
    sheet.appendRow(headers);
  ...
  try {
    if (data.familias)  writeBackupSheet('Familias',      ['id','n_alumno','n_apoderado'],                            data.familias);
    if (data.pagos)     writeBackupSheet('Pagos',         ['id','id_familia','mes','monto','fecha_pago','link_drive'], data.pagos);
    if (data.gastos)    writeBackupSheet('Gastos',        ['id','fecha','descripcion','valor','link_drive'],           data.gastos);
    if (data.perfiles)  writeBackupSheet('Perfiles',      ['id','nombre','permisos'],                                 data.perfiles.map(function(p) {
      return { id: p.id, nombre: p.nombre, permisos: JSON.stringify(p.permisos) };
    }));
    if (data.usuarios)  writeBackupSheet('Usuarios',      ['id','usuario','nombre_completo','perfil_id','activo'],    data.usuarios);
    if (data.configuracion && data.configuracion.anio) {
      writeBackupSheet('Configuracion', ['anio','monto','inicio','fin'], [data.configuracion]);
    }
...
function handleUploadFile(data) {
  try {
    ...
    var decoded  = Utilities.base64Decode(data.file);
    ...
    return { ok: true, link_drive: file.getUrl() };
  } catch(e) {
    return { ok: false, error: e.toString() };
  }
}
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'full_backup':    result = handleFullBackup(payload);      break;
      case 'upload_file':    result = handleUploadFile(payload);      break;
      default:
